    ...
    {bookings.map((item,index)=>(
      <div key={index} className='flex flex-col md:flex-row justify-between bg-primary/8 border border-primary/20 rounded-lg mt-4 p-2 max-w-3xl'>
        <div className='flex flex-col md:flex-row'>
          <img
            src={
              item.show && item.show.movie && item.show.movie.poster_path
                ? image_base_url + item.show.movie.poster_path
                : '/default-poster.png'
            }
            alt={item.show && item.show.movie ? item.show.movie.title : 'Movie Poster'}
          ...
          <div className='flex flex-col p-4'>
            <p className='text-lg font-semibold'>{item.show?.movie?.title || 'Unknown Title'}</p>
