...
const MovieDetails = () => {
  ...
  const handleFavorite = async () =>{
    try {
      if(!user){
        return toast("Please login to Add favorite")
      }
    ...
    <div>
          ...
          <div className='flex items-center gap-4 w-max px-4'>
            {
              show.movie.casts.slice(0, 12).map((cast, index) => (
                <div key={index} className='flex flex-col items-center texxt-center'>
      ...
      <p className='text-lg font-medium mt-20 mb-8 '>You May Also Like</p>
            ...
            <div className='flex justify-center mt-20'>
              <button onClick={()=>{navigate('/movies');scrollTo(0,0)}} className='px-10 py-3 text-sm bg-primary hove:bg-primary-dull transition rounded-md font-medium cursor-pointer'>Show More</button>
            </div>
    </div>
  ) : <div>
   <Loading />
